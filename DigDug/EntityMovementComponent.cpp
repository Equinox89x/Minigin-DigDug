...
void dae::EntityMovementComponent::Update()
{
	...
	SDL_Rect smallerRect{ m_Rect.x, m_Rect.y, m_Rect.w / 10, m_Rect.h / 10 };
	...
	m_LeftRect.y += static_cast<int>(m_Rect.h / 2.1f);

	m_RightRect.x += (m_Rect.w);
	m_RightRect.y += static_cast<int>(m_Rect.h / 2.1f);

	m_BottomRect.y += (m_Rect.h);
	m_BottomRect.x += static_cast<int>(m_Rect.w / 2.1f);

	m_TopRect.y -= m_TopRect.h;
	m_TopRect.x += static_cast<int>(m_Rect.w / 2.1f);
...
void dae::EntityMovementComponent::CheckMovement(const std::map<int, PathWay>& pathways)
{
	...
	for (auto item : m_CurrentSurroundingTiles)
	{
		...
		if (path != pathways.end()) {
			if (path->second.PathState == EPathState::Dug) {
